...
def test_synchronization():
    engine = QLSEngine(n_agents=100)
    result = engine.run_simulation(steps=500)
    assert result['coherence'] > 0.95, f"r={result['coherence']:.3f} < 0.95"
    assert result['ceml'] > 0.8, f"CEML={result['ceml']:.3f} < 0.8"

def test_scaling():
    engine_small = QLSEngine(10)
    engine_large = QLSEngine(496)
    r_small, _ = engine_small.simulator.simulate(100)
    r_large, _ = engine_large.simulator.simulate(100)
    assert r_large > r_small * 0.95  # Scaling stable
⚙️ 5. GitHub Actions (ci.yml)
text
name: CI
...
🎬 6. Commands de Lancement (copie-colle)
bash
# 1. Clone & setup
git clone https://github.com/quantum-lichen/QLS-Engine.git
cd QLS-Engine
...
# 4. Push (ton repo)
git add .
git commit -m "🚀 Initial QLS-Engine: Kuramoto+CEML 0.999 coherence"
